...
class _CustomSeekBarState extends State<CustomSeekBar> {
  ...
  void scrollMethod() {
    ...
    final M3U8Second? timestamp = bloc.timestampList
        .firstWhereOrNull((element) => element.second == currentTime.toInt());
    if (timestamp != null) {
      // logger
      //     .fine("lora lassan ${DateTime.parse(timestamp.timestamp).toLocal()}");
      bloc.updateVideoTimer(DateTime.parse(timestamp.timestamp));
    }
          ...
          _debounceScrollTimer =
            ...
            if (bloc.videoController!.value.isInitialized) {
              await bloc.videoController!
                  ...
                  .then((_) {
                logger.fine(
                  "🛑 User stopped scrolling with accurate time mapping: $seconds seconds -> ${realTimestamp.toIso8601String()}",
                );
                bloc.updateVideoTimer(realTimestamp);
                if (isPlay) {
                  Future.delayed(const Duration(seconds: 1), () {
  ...
  void _handleUserScroll() {
    ...
    if (bloc.videoController != null &&
      ...
      final M3U8Second? timestamp = bloc.timestampList
          .firstWhereOrNull((element) => element.second == seconds.toInt());
      if (timestamp != null) {
        bloc.updateVideoTimer(DateTime.parse(timestamp.timestamp));
      }
